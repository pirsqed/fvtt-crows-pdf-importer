...
export function setupImport(){
  ...
  const makeDoc=source=>{
    ...
    return {get id(){return data._id;},get name(){return data.name;},get type(){return data.type;},get system(){return data.system;},get img(){return data.img;},
      ...
      async deleteEmbeddedDocuments(type,ids){writes.push(['deleteEmbedded',this.id]);data.items=[];},
      async createEmbeddedDocuments(type,items){writes.push(['createEmbedded',this.id]);data.items=structuredClone(items);},
