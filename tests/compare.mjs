...
const location=new URL('../out/',import.meta.url);
const cases=JSON.parse(await readFile(process.env.CROWS_BASELINE ?? new URL('baseline.json',location),'utf8'));
...
await writeFile(new URL('../out/comparison.json',import.meta.url),JSON.stringify(summary,null,2));
