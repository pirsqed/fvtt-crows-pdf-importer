...
export function openPacketPreview(){
  ...
  dialog.innerHTML=`<header>
    ...
    <section>
      ...
      <p>Extracts inventory, creatures, traits, backgrounds, and NPC connection benefits. You will review and filter all entries before saving.</p>
  ...
  $('extract').onclick=async()=>{
    ...
    try{
      ...
      lastReport=report;$('open-import').disabled=Boolean(report.errors.length||report.backgroundIssues.length||report.validation.status==='failed'||report.actorValidation?.status==='failed'||!globalThis.game?.user?.isGM);
